...
void modem_test_USART0_Write_cb(const uint8_t data, int call_count){
 ...
 void modem_send_message_READY_test(){
    ...
    uint8_t fixture[31] = {0x7E, 0x00, 0x1B, 0x10, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFE, 0x00, 0x00, 0x11, 0x21, 0x12, 0x02, 0xC0, 0x2B, 0xE2, 0x09, 0xC0, 0x2D, 0xE2, 0x07, 0xC0, 0x41};

    printf("fixture=\t");
    for(uint8_t i=0; i<31;i++ ){
        printf("0x%02X, ", fixture[i]);
        expected[i] = fixture[i];
    }
    printf("\n");    
 ...
 void modem_send_message_DATA_test(){
    ...
    uint8_t fixture[46] = {0x7E, 0x00, 0x2A, 0x10, 0x01, 0x00, 0x00, 0x00, 
    ...
    modem_test_message = node_create_message(NODE_TOKEN_DATA, sid_);
